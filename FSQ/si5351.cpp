...
fprintf(stderr, "\nDEBUG: Si5351(0x%02xh, %d) fd=%d\n", i2c_bus_addr, i2c_bus_addr, fd);
	uint8_t reg_val=0;
...
// 2018-05-25 msipin - Why the heck is the following instruction commented out!?!?!?
//                     I validated that it is commented-out today in the source code
//                     repository.
// SEE: https://raw.githubusercontent.com/etherkit/Si5351Arduino/master/src/si5351.cpp
//
	//si5351_write(SI5351_PLL_INPUT_SOURCE, reg_val);
}
...
uint8_t Si5351::si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	...
	{
		//Wire.write(data[i]);
		wiringPiI2CWriteReg8(fd, addr+i, data[i]);
	}
